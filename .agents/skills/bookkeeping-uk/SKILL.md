---
name: bookkeeping-uk
description: Set up and run UK bookkeeping. Use when the founder asks about accounting software, bookkeeping, Xero, FreeAgent, QuickBooks, recording income and expenses, or getting ready for tax. Triggered around Phase 3.
---

# Bookkeeping (UK)

Good books are not optional — they are how the founder knows whether the venture is real,
and they are required for Making Tax Digital. Set this up at Phase 3, before the volume
makes it painful.

## Software choice
- **FreeAgent** — strong for sole traders and freelancers, with a UK-specific tax workflow.
  **Free** with a Mettle, NatWest, RBS, or Ulster business account (one transaction a month
  keeps it free). Recommended at the start if the founder banks with one of those.
- **Xero** — best for a growing SME: the widest UK accountant adoption and the
  most integrations (Shopify, Stripe, GoCardless, Dext). Recommended once VAT-registered or
  hiring. From roughly £16/month.
- **QuickBooks** — the cheapest sole-trader entry; fine, but fewer integrations than Xero.

**Recommendation:** start on FreeAgent (free via Mettle/NatWest); switch to Xero when
crossing the VAT threshold or making the first hire.

## What to set up
1. A separate business bank account — never mix personal and business money.
2. Connect the bank feed to the accounting software.
3. A simple chart of accounts; categorise transactions weekly, not yearly.
4. Keep digital copies of every receipt and invoice — MTD requires digital records.
5. Connect your sales platform and payment processor so sales reconcile automatically.

## Discipline
- Reconcile weekly — 15 minutes a week beats two lost days every January.
- Keep `state/financials.md` as the strategic source of truth; the software is the ledger.
- Set money aside for tax as it is earned — a separate pot for VAT and Corporation Tax.

## Verify
MTD-compatible software is mandatory as MTD-IT phases in (see `uk-tax-vat-mtd`). Check the
gov.uk recognised-software list before committing to a package.
