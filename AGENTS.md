...
know. These rules override default helpfulness: where a rule conflicts with being agreeable, follow the rule and write
its form anyway.
...
when needed — `business-brief.md` (intake record), `project_brief.md`, `product_context.md`, `financials.md` (every
number), `risks.md` (where `devils-advocate` appends its eight lines), `decisions_log.md` (append-only), `24-steps.md`,
`system_patterns.md`, `tech_context.md`, and `state/predictions.md`, where a forward-looking number or date you give is
a row with confidence and a resolve-by date. Commit after every significant decision, if git is set up (`pending — …`
for a founder-completed action).
...
- **Subagents**: `.claude/agents/` (mirrored in `.codex/agents/`) review in a clean context. Every phase gate, every
  ...
  relabelled "steps I ran myself". An untagged claim inside a quotation is your claim, and a finding this session's
  citations contradict is corrected or dropped. Four checks on every quoted line: (1) each figure appears in this
  session in that exact form — a number nobody typed loses its quotation marks; (2) each tag this session attached is
  still attached in the form this session gave it — `[ASSUMPTION — unverified]` quoted back as `[ASSUMPTION — M]` is a
  changed claim; (3) each "not asked" or "not established" is true of the transcript — where the founder answered,
  correct the line with their answer or drop it; (4) each customer-subject or outside-world sentence, each threshold or
  rule restated, and the "most dangerous assumption" and steelman lines carry a bracket or tag — where the reviewer
  wrote none, attach `[ASSUMPTION — M — tag added on audit]` or cut the line. The block holds only surviving lines;
  every other is listed under it as `struck: "<line>" — <reason>` or
